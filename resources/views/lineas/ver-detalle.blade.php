<x-app-layout>
    ...
    <div class=" bg-gray-200 border-gray-200 shadow rounded-md pb-5 sm:mx-5 lg:mx-20">
        <div class="flex items-center bg-white border-b-4 border-emerald-400 py-3">
            <h1 class="text-2xl font-semibold px-3 text-gray-800">Factura: {{ $factura->numero }}</h1>
        </div>
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 border-separate">
                ...
                @foreach ($factura->lineas as $linea)
                    @php
                        ...
                        $precio_sin_iva = $linea->precio * ((100 - $linea->iva) / 100);
                        $precio_con_iva = $linea->precio;
                        $precio_con_iva_descuento = $linea->precio * ((100 - $linea->descuento) / 100);
                        $precio_total_producto = $linea->cantidad * $precio_con_iva_descuento;
                    ...
                    <tr class="odd:bg-white even:bg-slate-100 border-b dark:even:bg-gray-900 dark:odd:bg-gray-800 dark:border-gray-700 hover:bg-emerald-50 dark:hover:bg-gray-600 transition duration-150 ease-in-out">
                        ...
                        <td class="px-6 py-4">
                            {{ numfmt_format_currency($fmt, $precio_sin_iva, 'EUR') }}
                        </td>
                        ...
                        <td class="px-6 py-4">
                            {{ numfmt_format_currency($fmt, $precio_con_iva, 'EUR') }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $linea->descuento }}%
                        </td>
                        ...
                        <td class="px-6 py-4">
                            {{ numfmt_format_currency($fmt, $precio_con_iva_descuento, 'EUR') }}
                        </td>

                        <td class="px-6 py-4">
                            {{ numfmt_format_currency($fmt, $precio_total_producto, 'EUR') }}
                        </td>

                    </tr>
                @endforeach
                <tr class="odd:bg-white even:bg-slate-100 border-b dark:even:bg-gray-900 dark:odd:bg-gray-800 dark:border-gray-700 hover:bg-emerald-50 dark:hover:bg-gray-600 transition duration-150 ease-in-out">
                    <td class="px-6 py-4 text-gray-800 dark:text-white font-bold text-right" colspan="7">TOTAL</td>
