        ...
        <article>
            ...
            <div class="flex flex-wrap space-x-2">
                ...
                <a href="{{ route('admin.productos.show', $valoracion->producto_id) }}">
                    <span
                        class="text-md font-medium text-gray-800 dark:text-gray-200 hover:text-emerald-500">{{ $valoracion->producto->denominacion }}</span>
                </a>
            </div>

            <footer class="mb-5 text-sm text-gray-500 dark:text-gray-400">
                <p>Comentado en:
                    {{ $valoracion->created_at->setTimeZone(new DateTimeZone('Europe/Madrid'))->format('d-m-Y H:i:s') }}
                </p>
                @if ($valoracion->created_at != $valoracion->updated_at)
                    <p>Última modificación:
                        {{ $valoracion->updated_at->setTimeZone(new DateTimeZone('Europe/Madrid'))->format('d-m-Y H:i:s') }}
                    </p>
                @endif
            </footer>
            <div class="pb-3">
            ...
            <div class="flex justify-between">
                <label id="label_comentario" for="comentario">Comentario:</label>
            ...
            <textarea name="comentario" id="comentario" rows="5" cols="100" required maxlength="250"
                class="w-full text-gray-500 dark:text-gray-400 rounded-lg border border-gray-300 resize-none">{{ old('comentario', $valoracion->comentario) }}</textarea>
